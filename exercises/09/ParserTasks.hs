...
import Data.Char
import Parser (Parser, nom, parse, parseFailure, parseMany)
import Prelude hiding ((<$>))

-- IO a
-- Parser a -
-- an effect which, when ran on some input string, produces a value of type @a@
...
optional px =
  (Just <$> px) <|> pure Nothing

--  where
--    -- foobar :: Parser (Maybe a)
--    foobar = do
--      x <- px
--      pure $ Just x

-- pure Nothing :: Parser (Maybe a)
-- px :: Parser a
-- (<|>) :: Parser a -> Parser a -> Parser a
-- foobar <|> pure Nothing
